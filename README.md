...
На основе этого roadmap в папке [`course/`](./course) собран детальный курс из 13 модулей. Каждый модуль разбит на уроки по схеме: **🎯 Цель → 📚 Теория → 💻 Практика → ✅ Чек → ⚠️ Красные флаги**.
...
1. Открой [Модуль 0](./course/00-fundamentals.md) и пройди его до конца, отмечая чекбоксы.
2. 2. Для каждого модуля выполняй проект до конца — незаконченный проект учит хуже законченного «плохого».
   3. 3. После каждого проекта сверяйся с чек-пойнтом в конце модуля.
      4. 4. Пройдя модули 0–7 — иди на первые Junior-собеседования.
         5. 5. Не стесняйся возвращаться к ранним модулям — глубина приходит со вторым проходом.
            6. 
